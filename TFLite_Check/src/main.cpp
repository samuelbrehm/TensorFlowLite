...
int main(int argc, char **argv)
{
    ...
    cv::putText(frame, "Infernce Time in ms: " + std::to_string(inference_time), cv::Point(15, 60), cv::FONT_HERSHEY_SIMPLEX, 1.3, cv::Scalar(0, 255, 0), 3);
    ...
    {
        const float confidence = result.first * 100;
        const int index = result.second;
        std::string output_txt = "Label :" + labels[index] + " Confidence : " + std::to_string(confidence) + "%";
        cv::putText(frame, output_txt, cv::Point(15, 110), cv::FONT_HERSHEY_SIMPLEX, 1.3, cv::Scalar(0, 255, 0), 3);
    }
